...
class Messenger_bot extends Home
{
    ...
    public function create_subscriber($sender_id='', $page_id='')
    {
        ...
        $select=array("id","unavailable_conversation","unavailable","subscribe_id","first_name","last_name","status","page_table_id","is_bot_subscriber","user_id","gender","contact_group_id","email","phone_number","birthdate","last_subscriber_interaction_time");
    ...
    public function webhook_callback_main()
    {
      ...
      // $response_raw='{"object":"page","entry":[{"id":"110464057210276","time":1619510315846,"messaging":[{"sender":{"id":"3200261786700724"},"recipient":{"id":"110464057210276"},"timestamp":1619510315662,"message":{"mid":"m_hdBTILE7ZkpB9jUp6W9X_Gwr8Hve7tCcflm4nvGFMgMdkwC_8Qqt8Ge_yg1Iu3doFAhiUrO9taeS_l4iDvaOlQ","text":"Hi"}}]}]}';
    ...
    public function template_manager()
    {
        ...
        foreach($page_info as $value)
        {
            $flow_page_list[$value['id']] = $value['page_name']." [".$value['name']."]";
        }
        $data['page_list'] = $flow_page_list;
    ...
    public function template_manager_data()
    {
        ...
        foreach ($info as $key => $value) 
        {
            ...
            {
                $action .= "<a class='btn btn-circle btn-outline-warning edit_reply_info' title='". $this->lang->line("Edit") ."' href='#' table_id='".$value['visual_flow_campaign_id']."' ><i class='fas fa-edit'></i></a>&nbsp;";
                $info[$i]["visual_flow_type"] = $this->lang->line('Flow Builder');
            }
            else
            {
                $action .= "<a class='btn btn-circle btn-outline-warning' title='". $this->lang->line("Edit") ."' href='".base_url('messenger_bot/edit_template/').$value['id']."'><i class='fas fa-edit'></i></a>&nbsp;";
                $info[$i]["visual_flow_type"] = $this->lang->line('Classic Editor');
            }

            $action .= "<a class='btn btn-circle btn-outline-primary' title='". $this->lang->line("Clone") ."' href='".base_url('messenger_bot/clone_template/').$value['id']."'><i class='far fa-copy'></i></a>&nbsp;<a href='#' class='btn btn-circle btn-outline-danger delete_template' title='".$this->lang->line("Delete")."' table_id='".$value['id']."'><i class='fa fa-trash'></i></a></div>";
