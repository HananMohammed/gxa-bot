...
class Subscriber_manager extends Home
{
    ...
    public function subscriber_actions_modal()
    {
      ...
      if($this->addon_exist("custom_field_manager"))
      {
        if($this->session->userdata('user_type') != 'Admin' && !in_array(292,$this->module_access))
        {
          $user_input_start_stop = '<a href="" class="dropdown-item has-icon reset_user_input_flow"  button_id ="'.$subscriber_data['id']."-".$subscriber_data['subscribe_id']."-".$subscriber_data['page_table_id'].'"><i class="fas fa-retweet"></i> '.$this->lang->line("Reset User Input Flow").'</a>';
        }
